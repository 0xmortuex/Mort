...
class Checker:
    ...
    def _infer_call(self, e):
        ...
        if e.name == "atomic_i64_compare_exchange":
            ...
            return "bool"
        if e.name in ("net_tcp_connect", "net_tcp_listen"):
            if self.freestanding:
                self._error("networking is not available in freestanding mode", e)
            expected_count = 2 if e.name == "net_tcp_connect" else 3
            if len(e.args) != expected_count:
                detail = "host and port" if expected_count == 2 else (
                    "host, port, and backlog")
                self._error(f"{e.name} expects {detail}", e)
            ...
            if expected_count == 3:
                self._check_expr(e.args[2])
                if not self._coerce("u32", e.args[2]):
                    self._error(
        ...
        if e.name in (
            ...
            if self.freestanding:
                self._error("networking is not available in freestanding mode", e)
            if len(e.args) != 1:
                self._error(f"{e.name} expects one socket handle", e)
            handle = self._check_expr(e.args[0])
            if handle != "*void":
                self._error(f"{e.name} handle must be *void, got {handle}", e)
            if e.name == "net_tcp_accept":
                return "*void"
            if e.name == "net_socket_close":
                return "void"
        ...
        if e.name in ("net_socket_send", "net_socket_recv"):
            ...
            self._check_expr(e.args[1])
            buffer_type = (
                "*const u8" if e.name == "net_socket_send" else "*u8")
            if not self._coerce(buffer_type, e.args[1]):
                self._error(
