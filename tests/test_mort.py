...
fn main() -> i64 {
    ...
    match strings.rsplit_once(slice("a=b=c" as *const u8, 5), slice("=" as *const u8, 1)) {
        Option<SplitOnce>.Some(parts) => {
            if parts.before.len == 3 && strings.equal(parts.before, slice("a=b" as *const u8, 3)) { ok += 1; }
            if parts.after.len == 1 && strings.equal(parts.after, slice("c" as *const u8, 1)) { ok += 1; }
        },
        Option<SplitOnce>.None => {},
    }

    match strings.rsplit_once(slice("file.tar.gz" as *const u8, 11), slice("." as *const u8, 1)) {
        Option<SplitOnce>.Some(parts) => {
            if parts.before.len == 8 && strings.equal(parts.before, slice("file.tar" as *const u8, 8)) { ok += 1; }
            if parts.after.len == 2 && strings.equal(parts.after, slice("gz" as *const u8, 2)) { ok += 1; }
        },
        Option<SplitOnce>.None => {},
    }

    match strings.rsplit_once(slice("noseparator" as *const u8, 11), slice("=" as *const u8, 1)) {
        Option<SplitOnce>.Some(parts) => {},
        Option<SplitOnce>.None => { ok += 1; },
    }

    match strings.rsplit_once(slice("abc" as *const u8, 3), slice("" as *const u8, 0)) {
        Option<SplitOnce>.Some(parts) => {
            if parts.before.len == 3 && strings.equal(parts.before, slice("abc" as *const u8, 3)) { ok += 1; }
            if parts.after.len == 0 { ok += 1; }
        },
        Option<SplitOnce>.None => {},
    }

    let empty: []const u8 = slice("" as *const u8, 0);
    match strings.rsplit_once(empty, slice("=" as *const u8, 1)) {
        Option<SplitOnce>.Some(parts) => {},
        Option<SplitOnce>.None => { ok += 1; },
    }
...
'''
    with tempfile.TemporaryDirectory() as d:
        source = os.path.join(d, "rsplitonceuse.mx")
        with open(source, "w", encoding="utf-8") as fh:
            fh.write(program)
        exe = os.path.join(d, "rsplitonceuse.exe" if os.name == "nt" else "rsplitonceuse")
        result = subprocess.run(
            [sys.executable, os.path.join(ROOT, "mortc.py"), source,
             "--run", "-o", exe],
            capture_output=True,
...
@needs_cc
def test_std_option_map_transforms_some_and_passes_through_none():
    # map consumes `value` via `match move` (see std/option.mx's doc
    # comment) and applies a plain top-level function reference to a Some
    # payload, wrapping the result back into Option<U>; a None value is
    # passed through unchanged (never calling `f`). Also checked with a
    # resource payload (Res), confirming the un-mapped None branch drops
    # the resource automatically and the Some branch's payload is
    # destroyed exactly once inside `extract_id` when it goes out of scope.
    program = r'''import std.option;
...
fn main() -> i64 {
    ...
    match option.map(Option<i64>.Some(21), double) {
        Option<i64>.Some(v) => { if v == 42 { ok += 1; } },
        Option<i64>.None => {},
    }

    match option.map(Option<i64>.None, double) {
        Option<i64>.Some(v) => {},
        Option<i64>.None => { ok += 1; },
    }

    let some_res: Option<Res> = Option<Res>.Some(Res { id: 7 });
    match option.map(move some_res, extract_id) {
        Option<i64>.Some(v) => { if v == 7 { ok += 1; } },
        Option<i64>.None => {},
    }

    let none_res: Option<Res> = Option<Res>.None;
    match option.map(move none_res, extract_id) {
        Option<i64>.Some(v) => {},
        Option<i64>.None => { ok += 1; },
    }
...
'''
    with tempfile.TemporaryDirectory() as d:
        source = os.path.join(d, "optionmap.mx")
        with open(source, "w", encoding="utf-8") as fh:
            fh.write(program)
        exe = os.path.join(d, "optionmap.exe" if os.name == "nt" else "optionmap")
        result = subprocess.run(
            [sys.executable, os.path.join(ROOT, "mortc.py"), source,
             "--run", "-o", exe],
            capture_output=True,
...
@needs_cc
def test_std_result_map_transforms_ok_and_passes_through_err():
    # Mirrors test_std_option_map_transforms_some_and_passes_through_none:
    # map consumes `value` via `match move` (see std/result.mx's doc
    # comment) and applies a plain top-level function reference to an Ok
    # payload, wrapping the result back into Result<Mapped, Error>; an Err
    # value is passed through unchanged (never calling `f`, error payload
    # preserved). Also checked with a resource Ok payload (Res), confirming
    # the Ok branch's payload is destroyed exactly once inside `extract_id`
...
fn main() -> i64 {
    ...
    match result.map(Result<i64, i64>.Ok(21), double) {
        Result<i64, i64>.Ok(v) => { if v == 42 { ok += 1; } },
        Result<i64, i64>.Err(e) => {},
    }

    match result.map(Result<i64, i64>.Err(-1), double) {
        Result<i64, i64>.Ok(v) => {},
        Result<i64, i64>.Err(e) => { if e == -1 { ok += 1; } },
    }

    let ok_res: Result<Res, i64> = Result<Res, i64>.Ok(Res { id: 7 });
    match result.map(move ok_res, extract_id) {
        Result<i64, i64>.Ok(v) => { if v == 7 { ok += 1; } },
        Result<i64, i64>.Err(e) => {},
    }

    let err_res: Result<Res, i64> = Result<Res, i64>.Err(-2);
    match result.map(move err_res, extract_id) {
        Result<i64, i64>.Ok(v) => {},
        Result<i64, i64>.Err(e) => { if e == -2 { ok += 1; } },
    }
...
'''
    with tempfile.TemporaryDirectory() as d:
        source = os.path.join(d, "resultmap.mx")
        with open(source, "w", encoding="utf-8") as fh:
            fh.write(program)
        exe = os.path.join(d, "resultmap.exe" if os.name == "nt" else "resultmap")
        result = subprocess.run(
            [sys.executable, os.path.join(ROOT, "mortc.py"), source,
             "--run", "-o", exe],
            capture_output=True,
...
fn main() -> i64 {
    ...
    // Odd length, reversed through the borrowed as_slice view.
    vec.push(&values, 1);
    vec.push(&values, 2);
    vec.push(&values, 3);
    vec.push(&values, 4);
    vec.push(&values, 5);
    let view: []i64 = vec.as_slice(&values);
    vec.slice_reverse(view);
    match vec.get(&values, 0) {
        Option<i64>.Some(x) => { if x == 5 { ok += 1; } },
        Option<i64>.None => {},
    }
    ...
    // Reversing twice restores the original order. The slice view stays
    // valid since slice_reverse doesn't reallocate the backing array.
    vec.slice_reverse(view);
    match vec.get(&values, 0) {
        Option<i64>.Some(x) => { if x == 1 { ok += 1; } },
        Option<i64>.None => {},
    ...
    // Empty slice: no-op, does not crash.
    let empty: Vec<i64> = vec.new<i64>();
    defer vec.destroy(&empty);
    vec.slice_reverse(vec.as_slice(&empty));
    if vec.is_empty(&empty) { ok += 1; }
    ...
    defer vec.destroy(&single);
    vec.push(&single, 42);
    vec.slice_reverse(vec.as_slice(&single));
    match vec.get(&single, 0) {
        Option<i64>.Some(x) => { if x == 42 { ok += 1; } },
        Option<i64>.None => {},
    }
...
'''
    with tempfile.TemporaryDirectory() as d:
        source = os.path.join(d, "slicereverse.mx")
        with open(source, "w", encoding="utf-8") as fh:
            fh.write(program)
        exe = os.path.join(d, "slicereverse.exe" if os.name == "nt" else "slicereverse")
        result = subprocess.run(
            [sys.executable, os.path.join(ROOT, "mortc.py"), source,
             "--run", "-o", exe],
            capture_output=True,
