...
> 🖥️ The kernel also has its own showcase repo: [**0xmortuex/MortOS**](https://github.com/0xmortuex/MortOS) — buildable standalone (it fetches this compiler automatically), and developed ahead of the vendored copy here. The standalone OS has grown **users, permissions, and `sudo`**, a **hierarchical MortFS v2** (directories, ownership, Unix permission bits), a **home launcher and a Settings app**, a shell with a **working directory, `$PATH`, and environment variables** — and it **speaks TCP/IP and serves a web page** over [mortnet](https://github.com/0xmortuex/mortnet), with its reusable bare-metal drivers (PCI, RTL8139, AC'97 audio, UHCI USB) extracted into [MortHardware](https://github.com/0xmortuex/MortHardware) and a package registry for its planned `mpkg` package manager in [mort-repo](https://github.com/0xmortuex/mort-repo). Everything in it is written in Mort — this compiler builds all of it.
...
- **Structured tasks:** move-only `std.task.TaskGroup` resources retain every
  child, provide cooperative sequentially consistent cancellation, join in
  spawn order, reject new work after cancellation, and automatically cancel
  and join remaining children at lexical scope exit.
